        ...
        public override string Message
        {
            get
            {
                ...
                if (innerException != null)
                {
                    ...
                    {
                        resultMessage.AppendLine(string.Format("- Entity of type \"{0}\" in state \"{1}\" has the following validation errors:",
                            validationError.Entry.Entity.GetType().FullName, validationError.Entry.State));

                        foreach (var error in validationError.ValidationErrors)
                        {
                            resultMessage.AppendLine(string.Format("-- Property: \"{0}\", Value: \"{1}\", Error: \"{2}\"",
                                error.PropertyName,
